...
const Sidebar: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    setNewRouter([...customRouter, ...AdminRoutes]);
  }, [rootCompetitions]);
      ...
      ({ name_var, url_var, icon_var, children }: any, key: number) => (
        <div key={key} className="w-full">
          {children && children.length > 0 ? (
            <div>
              <div
                className={`px-4 py-2.5 -my-1 cursor-pointer ${
                  activeDrop(url_var) || menuActive === url_var
                    ? "text-white"
                    : "text-gray-400"
                } hover:text-white flex justify-between items-center text-left transition duration-100 ease-in-out transform `}
                onClick={() => {
                  console.log("parent");
              ...
              <ul
                className={`${
                  (console.log(menuActive?.includes(url_var)),
                      ...
                      <Link
                        ...
                        className={`ml-1.5 px-4 py-2.5 rounded-lg my-1 hover:scale-105 ${
                          activePath(url_var)
                            ...
                            : "text-gray-400"
                        } hover:text-white block transition duration-150`}
                      >
                        <div className="flex items-center ">
                          {img_var ? (
                            <span className="h-6 w-6 mr-2 border rounded-full flex items-center justify-center bg-gray-100">
                              <img
              ...
              <Link
                ...
                className={`flex item-center px-4 py-2.5 -my-1 rounded-lg ${
                  activePath(url_var)
                    ? "text-white bg-secondary rounded"
                    : "text-gray-400"
                } hover:text-white cursor-pointer transition duration-100 ease-in-out transform hover:scale-105`}
              >
                <i className={`${icon_var} mr-3`}></i>
                <p>{name_var}</p>
              </Link>
          ...
          {children && children.length > 0 ? (
            <div>
              <div
                className={`mb-1 cursor-pointer ${
                  activeDrop(url_var) || menuActive === url_var
                    ...
                    : "text-gray-400"
                } hover:text-white mb-2.5 transition duration-300 ease-in-out transform hover:scale-150`}
              ...
              <ul
                className={`${
                  menuActive === url_var
                    ? "bg-dark absolute top-0 left-16 2xl:left-20 z-50 pl-2 pr-5 w-max rounded transition duration-500 ease-in-out transform hover:scale-105"
                    : "hidden"
                }`}
              >
                {children.map(
                    ...
                    <li key={key}>
                      <Link
                        ...
                        className={`px-4 py-2.5 rounded-lg my-1 ${
                          activePath(url_var)
                            ? "text-white bg-secondary rounded -mr-3"
                            : "text-gray-400"
                        } hover:text-white block transition duration-150`}
                      >
                        <div className="flex items-center">
            ...
            <Link
              ...
              onClick={() => setSidebarOpen(false)}
              className={`${
                activePath(url_var) ? "text-white scale-150" : "text-gray-400"
              } mb-2.5 hover:text-white transition duration-500 ease-in-out transform hover:scale-150`}
            >
              <i className={`${icon_var} `}></i>
            </Link>
          )}
  ...
  return (
    <>
      <div className={`z-20`}>
        {/* SIDEBAR MOBILE */}
        <div
          className={`fixed inset-0 bg-dark bg-opacity-60 z-40 md:hidden md:z-auto transition-opacity duration-200 ${
        ...
        >
          <div
            className={`overflow-y-auto overflow-x-hidden h-full flex flex-col items-center text-white text-left`}
          >
            <div
            ...
            <button
              ...
              className={`hidden md:flex absolute ${
                collapse
                  ? "left-[14.6rem] 2xl:left-[16.6rem]"
                  : "left-[3rem] 2xl:left-[3.5rem]"
              } top-6 justify-center items-center bg-gray-600 
